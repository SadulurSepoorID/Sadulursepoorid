...
function playIncomingRing(callback) {
    if (!isSfxOn) {
        if (callback) callback();
        return;
    }
    
    // Khusus untuk dering telepon masuk dari stasiun lain
    let radioAudio = new Audio('AUD-20230226-WA0054.mp3');
    let loopCount = 0;
    
    radioAudio.onended = function() {
        loopCount++;
        if (loopCount < 2) {
            radioAudio.currentTime = 0;
            radioAudio.play().catch(err => { if(callback) callback(); });
        } else {
            // Jika sudah 2x bunyi, jalankan suara penyiar
            if (callback) callback(); 
        }
    };
...
function updateCommUI() {
    ...
    // Update angka notif di semua tempat
    let badgeDesktop = document.getElementById('notif-badge');
    if(badgeDesktop) badgeDesktop.innerText = `(${count})`; 
    
    let badgeMobile = document.getElementById('notif-badge-mobile');
    if(badgeMobile) badgeMobile.innerText = `(${count})`;
    
    let tabBadge = document.getElementById('tab-notif-badge');
    if(tabBadge) tabBadge.innerText = `(${count})`;
    
    // Aktifkan efek kedip (blinking) di tombol yang sedang aktif
    let btnTelepon = document.getElementById('btn-telepon'); 
    let btnTeleponMobile = document.getElementById('btn-telepon-mobile'); 
    
    if (count > 0) { 
        if(btnTelepon) btnTelepon.classList.add('blinking'); 
        if(btnTeleponMobile) btnTeleponMobile.classList.add('blinking');
    } else { 
        if(btnTelepon) btnTelepon.classList.remove('blinking'); 
        if(btnTeleponMobile) btnTeleponMobile.classList.remove('blinking');
    }
...
const SVG_LABELS = [
    {x: 5, y: 7, text: "← J3 (KE KLDB)"},
    {x: 5, y: 13, text: "J2 (DARI KLDB) →"},
    {x: 95, y: 7, text: "← J3 (DARI KRI)", anchor: "end"},
    {x: 95, y: 13, text: "J2 (KE KRI) →", anchor: "end"},
...
function checkTimetable() {
    TIMETABLE.forEach(t => {
        if (t.isCommuter && gameTime >= t.spawnTime - 30 && !t.hasCalled) {
            ...
            playIncomingRing(() => {
                speakVoice(`Panggilan masuk dari Stasiun ${fromStation}. ${voiceMsg}`, 0);
            });
        }
